...
import operator
from gil.lgp.logic.planner import LogicPlanner
from gil.lgp.geometry.kinematics import PointObject
from gil.lgp.geometry.workspace import YamlWorkspace, HumoroWorkspace
from gil.lgp.geometry.trajectory import linear_interpolation_waypoints_trajectory
from gil.lgp.geometry.geometry import get_closest_point_on_circle, get_point_on_circle
from gil.lgp.optimization.objective import TrajectoryConstraintObjective
...
class LGP(object):
    ...
    def __init__(self, domain, problem, workspace_config, **kwargs):
        ...
        self.logic_planner = LogicPlanner(domain)
        #Add to fix problem
        self.logic_planner.init_planner(problem=problem, ignore_cache = True)
    ...
    def plan(self):
        ...
        print(len(act_seqs))
        for i in np.arange(len(act_seqs[0])):
            print("Act ",i, " ", act_seqs[0][i])
        #Added end
        if self.verbose:
    ...
    # Added to fix
    def action_precondition_check(self, action):
        ...
        if self.plan is not None:  # current symbolic plan is still feasible, continue with this plan
            ...
            for r in self.ranking:
                if self.enable_viewer:
                    ...
                    success = status.value
                    traj = Trajectory(q_init=np.array(traj[:2]), x=np.array(traj[2:]))
                    #print("trajectorypath", traj.active_segment())
                else:
                    success, traj = self.objectives[r[1]].optimize()
                if success:  # choose this plan
                    self.plan = self.plans[r[1]]
